...
import { sendSignedTransaction } from '../utils'
...
export const getAssociatedTokenAddress = async function (
  wallet: PublicKey,
  mint: PublicKey
) {
  return (
...
export const getFairLaunchRaffleBitmap = async (
  tokenMint: anchor.web3.PublicKey
): Promise<[anchor.web3.PublicKey, number]> => {
  return await anchor.web3.PublicKey.findProgramAddress(
    [Buffer.from('fair_launch'), tokenMint.toBuffer(), Buffer.from('raffle')],
    FAIR_LAUNCH_PROGRAM_ID
  )
}
...
export async function getTokenAmount(
  ...
  if (!mint.equals(WRAPPED_SOL_MINT)) {
    try {
      ...
      amount =
        (token?.value?.uiAmount ?? 0) * Math.pow(10, token.value.decimals)
    } catch (e) {
      console.error(e)
      console.info(
        'Account ',
