...
import { useAllRaffleBuyersLeadersPaginated } from '../../techRaffles/hooks/user'
import { TBuyerLeaderboardStatsUser } from '../../techRaffles/routers/userRouter'
...
const LeaderboardBuyers: FC<Props> = ({ visible, timeframe }) => {
  const { colorMode } = useColorMode()
  const isDarkMode = colorMode === 'dark'

  const { data: buyersLeaderboardData, isFetching } = useAllRaffleBuyersLeadersPaginated(0, timeframe)
      ...
      {visible && (
        <>
          ...
          >
            ...
            <GridItem colSpan={[2]}>
              <Text fontWeight='600'>Tickets bought</Text>
            </GridItem>
            <GridItem colSpan={[2]}>
              <Text fontWeight='600'>Participated</Text>
            </GridItem>
            <GridItem colSpan={[2]}>
              <Text fontWeight='600'>Won</Text>
            </GridItem>
            <GridItem colSpan={[2]}>
              <Text fontWeight='600'>Volume</Text>
            </GridItem>
            {buyersLeaderboardData && buyersLeaderboardData.map((user, index) => (
              <>
                <GridItem colSpan={[3]}>
                  <HStack gap='.5rem'>
          ...
          {isFetching && (
            <Center color={isDarkMode ? 'white' : 'black'} mt='3rem'>
              Loading top buyers... <Spinner ml='1rem' color={isDarkMode ? 'white' : 'black'} size='sm'></Spinner>
            </Center>
          )}
        </>
      )}
...
export default LeaderboardBuyers
